...
git pull
sed -i.bak "s|.*printstatus=.*|printstatus=$stat|" post_one.f90
sed -i.bak "s|.*simlabel=.*|simlabel='$label'|" post_one.f90
sed -i.bak "s|.*width=.*|width=$width|" post_one.f90
sed -i.bak "s|.*lambda=.*|lambda=$wave|" post_one.f90
sed -i.bak "s|.*depth=.*|depth=$depth|" post_one.f90
sed -i.bak "s|.*amprat=.*|amprat=$amp|" post_one.f90
sed -i.bak "s|.*call handletopo(.*|call handletopo('$topo', XXX, YYY, ZZZ)|" post_one.f90
...
ifort var_3d.o postmod.o formatmod.o max_out.o headermod.o average.o column.o richardson.o massinchannel.o entrainment.o findhead.o constants.o openbin.o openascii.o allocate_arrays.o handletopo.o post_one.f90  -convert big_endian -o post.exe
