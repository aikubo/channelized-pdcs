...
INTEGER::fid_EP_G_t,fid_U_t,fid_ISO3,fid_Ri,fid_ROP1,fid_Dot
...
DOUBLE PRECISION, ALLOCATABLE :: ROP_S1(:,:),ROP_S2(:,:)
...
!OPEN(105,FILE='C_PG',form='unformatted')
!OPEN(106,FILE='C_PS1',form='unformatted')
!OPEN(107,FILE='C_PS2',form='unformatted')

OPEN(108,FILE='T_S1',form='unformatted')
!OPEN(109,FILE='T_S2',form='unformatted')
OPEN(110,FILE='ROP_S1',form='unformatted')
!OPEN(111,FILE='ROP_S2',form='unformatted')
!------------OPEN the binary files-----------!

!------------OPEN the topography files-------!
OPEN(600,FILE='topography',form='formatted')
OPEN(601,FILE='topo2',form='formatted')
OPEN(602,FILE='topo_M20_D18')
!------------OPEN the topography files-------!
...
DO t=1,timesteps
  ...
  !num_open = 950+t
  !OPEN(num_open,FILE='GRAD_Avg_Velocity_t'//trim(x1)//'.txt')

  num_open = 950+t
  OPEN(num_open,FILE='Local_Location_t'//trim(x1)//'.txt')

  !num_open = 1000+t
  !OPEN(num_open,FILE='GRAD_Entrainment_t'//trim(x1)//'.txt')
  num_open = 1050+t
  ...
  num_open = 1100+t
  OPEN(num_open,FILE='EP_G_t'//trim(x1)//'.txt')
  ...
  num_open = 1350+t
  !OPEN(num_open,FILE='ROP_C_P_S1_t'//trim(x1)//'.txt')

  num_open = 1400+t
  !OPEN(num_open,FILE='ROP_C_P_S2_t'//trim(x1)//'.txt')
...
ALLOCATE( channel_topo(RMAX-4,ZMAX-4))
...
!ALLOCATE( T_S1(length1,timesteps))
!ALLOCATE( T_S2(length1,timesteps))
ALLOCATE( ROP_S1(length1,timesteps))
!ALLOCATE( ROP_S2(length1,timesteps))

!ALLOCATE( C_PS1(length1,timesteps))
!ALLOCATE( C_PS2(length1,timesteps))
!ALLOCATE( C_PG(length1,timesteps))
...
DO t=1,timesteps
       ...
       DO I=1,RMAX*ZMAX*YMAX
          ...
          WRITE(fid_EP_G_t,401) EP_G1(I,t),XXX(I,1),YYY(I,1),ZZZ(I,1),current_density
